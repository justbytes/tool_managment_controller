...
import WONumber from '../models/WONumber'
...
const GenerateWONumber = async (data: WorkOrder): Promise<WorkOrder> => {
    ...
    async function checkWONumberExists(valueToCheck: string): Promise<boolean> {
        try {
          const result = await WONumber.findOne({
            where: { number: valueToCheck }
          });
    ...
    const day: number = Math.floor(JDStart + startOfToday.getTime() / dayMilliSeconds);
